...
namespace Card
{
    public class HangTianV1FenTi : ICard
    {
        //初始化串口
        ...
        private static Log Log = Log.GetInstance("Card.HangTianV1FenTi");
        ...
        public int ReadGasCard(short com, int baud, ref string kh, ref int ql, ref decimal money, ref short cs, ref short bkcs, ref string yhh)
        {
            ...
            Log.Debug("读卡结果:"+ret);
            //卡号
            if(cardno[0] == '0' && cardno[1] == '1' && cardno[2] == '0' && cardno[3] == '0' && cardno[4] == '0')
            {
                kh = Encoding.ASCII.GetString(cardno, 0, 14);
            }
            else
            {
        ...
        public int WriteGasCard(short com, int baud, ref string kmm, string kh, string dqdm, int ql, int csql, int ccsql, short cs, int ljgql, int bjql, int czsx, int tzed, string sqrq, string cssqrq, int oldprice, int newprice, string sxrq, string sxbj, string result)
        {
	        ...
	        if(ql != 0)
	        {
                ...
                results = StaticSellGas(com, baud,
                    cardno, ql,
                    0, cs, 2, null);
